...
## 接入点
接入点位于节点池之后。页面使用单列卡片列表，每行一个接入点；卡片按自然阅读顺序展示端口、运行状态、已启用能力和代理认证策略，不在卡片中划分多列信息区。

默认接入点显示只读锁定状态。自定义接入点提供编辑与删除图标操作；新建和编辑使用右侧 Drawer，包含端口、管理页面、代理总开关、协议开关及“当系统未设定代理令牌时，也强制客户端发送代理认证信息”。
